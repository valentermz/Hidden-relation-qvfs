...
-- The elimination ideal has a single generator
elimG = selectInSubring(1, G);

-- h is the hidden relation
h = elimG_(0,0)
...
-- Recover the coefficients in h = H_2*L^2 + H_1*L + H_0
H_0 = sub(h,{L=>0});
H_1 = sub(diff(L,h),{L=>0});
H_2 = sub(diff(L,diff(L,h))/2,{L=>0});
sub(diff(L,diff(L,diff(L,h)))/6,{L=>0}) == 0  --true
